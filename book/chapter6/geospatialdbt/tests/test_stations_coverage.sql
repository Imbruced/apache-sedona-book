WITH stations_with_observations AS (
    SELECT s.*
    FROM {{ ref('stations') }} as s
    JOIN {{ ref('observations') }} as o ON s.sampling_id = o.sampling_id
),
grid_all AS (
    SELECT
        country,
...
grid_with_measurement AS (
    SELECT distinct g.h3, g.country
    FROM {{ ref('grid') }} AS g
    JOIN stations_with_observations AS s ON ST_Intersects(s.geom, g.geom)
